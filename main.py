from fastapi import FastAPI, Query, status, HTTPException, Path, Body, Form, File, UploadFile
...
def get_single_item(name_id: Annotated[int, Path(alias="item_id", examples=[2])]):
...
@app.get("/names")
def search_items(q: Annotated[str | None, Query(deprecated= True, alias="search", description= "Searches The Word You Provided", examples=["Grand"], min_length=2, max_length=50)] = None):
...
def create_item(name: Annotated[str, Body(embed=True)]):
...
def update_single_item(name_id: int, name:str):
    for game in game_list:
        if game["id"] == name_id:
            game["name"] = name
            return game
